...
"""
from dataset_classes import InstitutionSalaries
...
def return_max_salary(data: list[InstitutionSalaries], salary_type: str) -> list:
    ...
    if salary_type == 'Median':
        for date in years:
            max_salary = max(salaries.median_salary for salaries in data
                             if salaries.ref_date == date)
        ...
        for date in years:
            max_salary = max(salaries.tenth_percentile_salaries for salaries in data
                             if salaries.ref_date == date)
            list_so_far.append(max_salary)
    elif salary_type == 'Ninetieth Percentile':
        for date in years:
            max_salary = max(salaries.ninetieth_percentile_salaries for salaries in data
                             if salaries.ref_date == date)
            list_so_far.append(max_salary)
    return list_so_far
...
def return_min_salary(data: list[InstitutionSalaries], salary_type: str) -> list:
    ...
    if salary_type == 'Median':
        for date in years:
            min_salary = min(salaries.median_salary for salaries in data
                             if salaries.ref_date == date)
        ...
        for date in years:
            min_salary = min(salaries.tenth_percentile_salaries for salaries in data
                             if salaries.ref_date == date)
            list_so_far.append(min_salary)
    elif salary_type == 'Ninetieth Percentile':
        for date in years:
            min_salary = min(salaries.ninetieth_percentile_salaries for salaries in data
                             if salaries.ref_date == date)
            list_so_far.append(min_salary)
    return list_so_far


def calculate_salary_percentage(list_of_institutions: list[InstitutionSalaries],
    ...
    for i in range(len(list_of_institutions) - 1):
        if salary_type == 'Average':
            percentage = calculate_percentage(list_of_institutions[i].avg_salary,
                                              list_of_institutions[i + 1].avg_salary)
            percentages.append(percentage)
        elif salary_type == 'Median':
            percentage = calculate_percentage(list_of_institutions[i].median_salary,
                                              list_of_institutions[i + 1].median_salary)
            percentages.append(percentage)
        elif salary_type == 'Tenth Percentile':
            percentage = calculate_percentage(list_of_institutions[i].tenth_percentile_salaries,
                                              list_of_institutions[i + 1].tenth_percentile_salaries)
            percentages.append(percentage)
        elif salary_type == 'Ninetieth Percentile':
            percentage = calculate_percentage(list_of_institutions[i].ninetieth_percentile_salaries,
                                              list_of_institutions[i + 1].
                                              ninetieth_percentile_salaries)
            percentages.append(percentage)
    return percentages


def calculate_percentage(value1: float, value2: float) -> float:
...
if __name__ == '__main__':
    ...
    python_ta.check_all(config={
        'extra-imports': ['hours_data_class'],  # the names (strs) of imported modules
